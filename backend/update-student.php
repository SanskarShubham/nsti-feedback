...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['submit'])) {
    // Get POST values
    $id = $_POST["id"];
    $attendence_id = trim($_POST["attendence_id"]);
    $name = trim($_POST["username"]);
    $trade = trim($_POST["trade"]);
    $program = $_POST["program"];

    // Update query
    $updateSql = "UPDATE students SET attendence_id = ?, name = ?, trade = ?, program = ? WHERE id = ?";
    $stmt = $conn->prepare($updateSql);
    $stmt->bind_param("ssssi", $attendence_id, $name, $trade, $program, $id);
