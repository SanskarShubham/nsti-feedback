...
$where = "WHERE 1";
if ($teacher !== '') $where .= " AND t.name = '" . mysqli_real_escape_string($conn, $teacher) . "'";
if ($subject !== '') $where .= " AND s.name = '" . mysqli_real_escape_string($conn, $subject) . "'";
if ($trade !== '') $where .= " AND tr.trade_name = '" . mysqli_real_escape_string($conn, $trade) . "'";
if ($rating !== '') $where .= " AND f.rating = " . (int)$rating;
...
$teacher_list = mysqli_query($conn, "SELECT DISTINCT name FROM teachers ORDER BY name");

// Fetch subject list for dropdown
$subject_list = mysqli_query($conn, "SELECT DISTINCT name FROM subject ORDER BY name");
?>
        ...
        <div class="card-body">
            ...
            <form method="GET" class="form-inline flex-wrap gap-2">
                ...
                <select name="teacher" class="form-control form-control-m mr-3 mb-2" style="max-width:200px;">
                    ...
                    <?php while ($t = mysqli_fetch_assoc($teacher_list)): ?>
                        <option value="<?= $t['name'] ?>" <?= $teacher == $t['name'] ? 'selected' : '' ?>>
                            <?= $t['name'] ?>
                        </option>
                    <?php endwhile; ?>
                </select>
                ...
                <select name="subject" class="form-control form-control-m mr-3 mb-2" style="max-width:200px;">
                    ...
                    <?php while ($s = mysqli_fetch_assoc($subject_list)): ?>
                ...
                <select name="trade" class="form-control form-control-m mr-3 mb-2" style="max-width:200px;">
                    ...
                    <?php
                    // Reset trade list pointer if needed
                    mysqli_data_seek($trade_list, 0);
                    while ($t = mysqli_fetch_assoc($trade_list)): ?>
                ...
                <!-- Rating Dropdown - Updated with "All" default option -->
                <select name="rating" class="form-control form-control-m mr-3 mb-2" style="max-width:120px;">
                    <option value="" <?= !isset($_GET['rating']) || $_GET['rating'] === '' ? 'selected' : '' ?>>All Ratings</option>
                    <?php for ($i = 1; $i <= 5; $i++): ?>
                        <option value="<?= $i ?>" <?= isset($_GET['rating']) && $_GET['rating'] == $i ? 'selected' : '' ?>><?= $i ?> ⭐</option>
                    <?php endfor; ?>
                </select>
