...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_feedback'])) {
  ...
  if (feedbackExists($conn, $attendance_id)) {
    echo "<script>
            showNotification('Feedback already submitted for this student.', 'error');
          </script>";
    exit;
  }
  ...
  if (!empty($errors)) {
    $error_str = implode("\n", $errors);
    echo "<script>
            showNotification('Please fill all mandatory fields:\\n$error_str', 'error');
          </script>";
    exit;
  }
  ...
  if (empty($insert_errors)) {
    ...
    echo "<script>
            showNotification('Error submitting feedback: $error_str', 'error');
          </script>";
  }
  exit;
}
...
<body>
  ...
  <script>
    ...
    // Enhanced notification for success
    function showSuccessNotification() {
      ...
      notification.innerHTML = `
        ...
        <div>
          ...
          <p>Thank you for your valuable feedback. Your input helps us improve.</p>
        </div>
      `;
    ...
    document.getElementById('fetchInfoBtn').addEventListener('click', function () {
      ...
      if (!attendanceId) {
        showNotification('Please enter Attendance ID', 'error');
        return;
      }
        ...
        .then((data) => {
          if (data.status === 'success') {
            ...
            if (teacherList.length === 0) {
              container.innerHTML = '<div class="notification info"><i class="fas fa-info-circle"></i> No teachers found for this trade.</div>';
            } else {
    ...
    document.getElementById('feedbackForm').addEventListener('submit', function (e) {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        if (!ratingChecked) {
          showNotification(`Please select rating for teacher #${i + 1}`, 'error');
          valid = false;
          break;
        }
        ...
        if (!remarks.value.trim()) {
          showNotification(`Please enter remarks for teacher #${i + 1}`, 'error');
          valid = false;
          break;
        }
      }
