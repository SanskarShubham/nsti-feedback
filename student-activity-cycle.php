...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['create_cycle'])) {
        ...
        if (!empty($cycle_name) && !empty($_POST['start_date']) && !empty($_POST['end_date'])) {
            if ($start_date >= $end_date) {
                ...
                $stmt = $conn->prepare("INSERT INTO student_activity_cycle (cycle_name, start_date, end_date, is_active) VALUES (?, ?, ?, 0)");
                $stmt->bind_param("sss", $cycle_name, $start_date, $end_date);
                if ($stmt->execute()) {
                    $message = "Activity cycle created successfully!";
    ...
    if (isset($_POST['edit_cycle'])) {
        ...
        if (!empty($cycle_name) && !empty($_POST['edit_start_date']) && !empty($_POST['edit_end_date']) && !empty($cycle_id)) {
             if ($start_date >= $end_date) {
                ...
                $stmt = $conn->prepare("UPDATE student_activity_cycle SET cycle_name = ?, start_date = ?, end_date = ? WHERE cycle_id = ?");
                $stmt->bind_param("sssi", $cycle_name, $start_date, $end_date, $cycle_id);
                 if ($stmt->execute()) {
                    $message = "Activity cycle updated successfully!";
                    $message_type = 'success';
                } else {
    ...
    if (isset($_POST['update_status'])) {
        ...
        try {
            ...
            if ($new_status == 1) {
                $conn->query("UPDATE student_activity_cycle SET is_active = 0");
            }
            
            // Now, update the selected cycle's status.
            $stmt = $conn->prepare("UPDATE student_activity_cycle SET is_active = ? WHERE cycle_id = ?");
...
$cycles = [];
$result = $conn->query("SELECT * FROM student_activity_cycle ORDER BY start_date DESC");
if ($result) {
...
<head>
    ...
    <title>Manage Student Activity Cycles</title>
    ...
    <style>
        ...
        .beautiful-title {
            ...
            line-height: 1.2; /* Fix for text being cut off */
        }
    </style>
</head>
<body>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title beautiful-title">Manage Student Activity Cycles</h4>
                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">Create New Activity Cycle</h5>
                                    </div>
                                    <div class="card-body">
                                        <form method="POST">
                                            <div class="form-group">
                                                <label for="cycle_name">Cycle Name</label>
                                                <input type="text" id="cycle_name" name="cycle_name" class="form-control" placeholder="e.g., Monthly Activity Report - August 2025" required>
                                            </div>
                                            <div class="form-row">
                                ...
                                <div class="table-responsive">
                                    <h5 class="mt-4">Existing Activity Cycles</h5>
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        <tbody>
                                            <?php if (empty($cycles)): ?>
                                                <tr>
                                                    <td colspan="5" style="text-align: center;">No activity cycles found.</td>
                                                </tr>
                                            <?php else: ?>
                                                <?php foreach ($cycles as $cycle): 
                                                    ...
                                                    } elseif ($cycle['is_active'] == 1 && $now >= $start) {
                                                        $status_text = 'Active';
    ...
    <div id="editCycleModal" class="modal">
        <div class="modal-content card">
            <div class="modal-header">
                <h5 class="modal-title">Edit Activity Cycle</h5>
