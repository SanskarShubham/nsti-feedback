...
if (isset($_POST['find_students'])) {
    ...
    if (!empty($selected_program) && !empty($selected_trade_id)) {
        // MODIFICATION: Fetch the new 'remarks' column
...
if (isset($_POST['save_activities'])) {
    ...
    $remarks = $_POST['remarks']; // Get the new remarks data
    
    $created_by = 1; // Example user ID
    $updated_by = 1; // Example user ID

    // MODIFICATION: Add 'remarks' to the INSERT and UPDATE statements
    ...
    foreach ($student_ids as $key => $student_id) {
        // MODIFICATION: Update bind_param to include remarks (string 's')
        $stmt->bind_param(
            "iiiiiisii", // i for int, s for string (remarks)
            $student_id,
            $total_lessons[$key],
            $total_demos[$key],
            ...
            $total_tests[$key],
            $total_tmps[$key],
            $remarks[$key], // Bind the remarks data
            $created_by,
            $updated_by
        );
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" class="form-control" name="remarks[]" value="<?php echo htmlspecialchars($student['remarks'] ?? ''); ?>">
                                </td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
